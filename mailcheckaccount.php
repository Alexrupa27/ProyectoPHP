...
<body>
    <div class="container">
        ...
        $activat = $db->prepare("SELECT active FROM usuari WHERE mail = :mail");
        $activat->bindParam(':mail', $mail);
        $activat->execute();
        $activatResult = $activat->fetch(PDO::FETCH_ASSOC);

        if ($activatResult && $activatResult['active'] == 1) {
            echo "<p class='error'>La cuenta ya ha sido activada.</p>";
            exit();
        }

        // Si la cuenta no está activada, verificamos el código de activación
